...
def spikes_and_stimulus(spikes, stimuli):
    ...
    for stimulus in range (len(stimuli)):
        for cell in range(len(spikes)):
            begin_idx[stimulus, cell] = bisection(spikes['Spiketimestamps'].loc[cell]
                                                   [:spikes['Nr of spikes'].loc[cell]], stimuli['Begin_Fr'][stimulus])
            end_idx[stimulus, cell] = bisection(spikes['Spiketimestamps'].loc[cell]
                                                   [:spikes['Nr of spikes'].loc[cell]], stimuli['End_Fr'][stimulus])
    ...
    for stimulus in range (len(stimuli)):
        ...
        for cell in range(nr_cells):
            ...
            if end_idx[stimulus, cell]==-1:
                ...
                stimulus_spikes[:nr_spikes_stimulus
                                [stimulus, cell], cell] = spikes['Spiketimestamps'].loc[cell][begin_idx[stimulus, cell]:
                                                                          end_idx[stimulus, cell]]
                # Substract stimulus begin Fr so spikes are relative to stimulus frame
                stimulus_spikes[:nr_spikes_stimulus[stimulus, cell], cell] = stimulus_spikes[:nr_spikes_stimulus
                                                                                             [stimulus, cell], cell]-begin_frs[stimulus]
                
                masked_spikes[nr_spikes_stimulus[stimulus, cell]:, cell] = np.ones((max_spikes_stimulus[stimulus]-
                                                                                    nr_spikes_stimulus[stimulus, cell]))
...
def get_spikes_per_trigger_type(spikes_df, trigger_complete, cell, trigger_index, repeat_log):
    """
    ...
    trigger_increase = np.linspace(0, nr_trigger-trigger_per_repeat, nr_repeats, dtype=int)+trigger_index
    
    
    # Calculate real trigger windows
    trigger_begin, trigger_end, interval = real_trigger_windows(trigger_complete, repeat_log)
    ...
    for repeat in range(nr_repeats):
        begin_idx, end_idx = find_trigger_spikes(np.array([trigger_begin[repeat], trigger_end[repeat]]), spikes)
        ...
        if repeat == 0:
            spiketrains_repeat=[spk.SpikeTrain(spikes_relative, edges=(0.0, interval))]
        else:
            spiketrains_repeat.append(spk.SpikeTrain(spikes_relative, edges=(0.0, interval)))
...
def get_spikes_whole_stimulus(spike_df, trigger_complete, cell, repeat_logic, freq=1):
    ...
    """
    trigger_begin, trigger_end, interval = real_trigger_windows(trigger_complete, repeat_logic)
    ...
    for trigger in range(repeat_logic):
        spikes_repeat, spiketrains_repeat = get_spikes_per_trigger_type(spike_df, trigger_complete, cell, trigger, repeat_logic)
    ...
    for repeat in range(nr_repeats):
        for trigger in range(len(all_spikes)):
            if trigger == 0:
                ...
                all_spiketrains[trigger][repeat].spikes = all_spiketrains[trigger][repeat].spikes/freq
                spiketrain_temp = all_spiketrains[trigger][repeat]
            else:
                temp_spikes = np.append(temp_spikes, all_spikes[trigger][repeat]+relative_begin[trigger])
                spiketrain_temp.spikes = np.append(spiketrain_temp.spikes, (all_spiketrains[trigger][repeat].spikes
                                                                            +relative_begin[trigger])/freq)
...
def plot_raster(spiketrain):
    ...
    test_raster = plotly.subplots.make_subplots(rows=2, cols=1,subplot_titles=("Spiketrains"), shared_xaxes=True)
    ...
    for repeat in range(len(spiketrain)):
        ...
        test_raster.add_trace(go.Scattergl(mode='markers', x=spikes_temp, y=yvalue,
    ...
    test_raster.add_trace(go.Scatter(x=x, y=y,name='Average Isi distance',
...
def plot_raster_whole_stimulus(spiketrains, repeat_logic, sublogic, axcolors, LED_names):
    ...
    whole_stim_raster = plotly.subplots.make_subplots(rows=6, cols=1,subplot_titles=
                                                      ("Spiketrains", "Isi Distance", "Spike Profile", "Spike Synchronization", "PSTH"),
                                                      shared_xaxes=True)
    ...
    for repeat in range(len(spiketrains)):
        ...
        nr_spikes = np.shape(spikes_temp)[0]
        yvalue = np.ones(nr_spikes)*repeat
        whole_stim_raster.add_trace(go.Scattergl(mode='markers', x=spikes_temp, y=yvalue,
                                               name=None,
                                            marker=dict(color='Black', size=2)))
    ...
    whole_stim_raster.add_trace(go.Scatter(x=x, y=y,name='Average Isi distance',
                            line=dict(color='Black')),
                            row=2, col=1)
    ...
    whole_stim_raster.add_trace(go.Scatter(x=x1, y=y1,name='Spike Profile',
                            line=dict(color='Black')),
                            row=3, col=1)
    ...
    whole_stim_raster.add_trace(go.Scatter(x=x2, y=y2,name='Spike synchronization',
                            line=dict(color='Black')),
                            row=4, col=1)
    
    x3, y3 = psth.get_plottable_data()
        
    whole_stim_raster.add_trace(go.Scatter(x=x3, y=y3,name='PSTH',
                            line=dict(color='Black')),
                            row=5, col=1)
    ...
    for i in range(nr_stim):
        #print(i)
        whole_stim_raster.add_trace(go.Scatter(x=[trigger_dur*i, trigger_dur*i,
                                    trigger_dur*(i+1), trigger_dur*(i+1), trigger_dur*i],
                                    y=[0, 1, 1, 0, 0], fill="toself",
                                    fillcolor=axcolors[i], name=LED_names[i], line=dict(
                                    width=0), mode="lines"), row=6, col=1)
